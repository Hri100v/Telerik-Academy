function solve() {
    $.fn.datepicker = function () {
        ...
        var picker = $('<div />').addClass('datepicker-content');
        var controls = $('<div />').addClass('controls').appendTo(picker);

        var buttonLeft = $('<button />').addClass('btn btn-previous').text('<').attr('data-operation', '-1').appendTo(controls);
        var innerMonth = $('<div />').addClass('current-month').text(currentDate.getMonthName() + ' ' + currentDate.getFullYear()).appendTo(controls);
        var buttonRight = $('<button />').addClass('btn btn-next').text('>').attr('data-operation', '1').appendTo(controls);
        ...
        $(document).click(function (ev) {
            if (!$(ev.target).parents('.datepicker-wrapper').length) {
                if (picker.hasClass('datepicker-content-visible')) {
                    togglePickerVisibility();
                }
            }
        });
        ...
        function togglePickerVisibility() {
            picker.toggleClass('datepicker-content-visible');
        }
