...
namespace RotatingWalkInMatrix
{
    public class SquareMatrix
    {
        ...
        public void RotatingWalkFill()
        {
            ...
            int rowChange = RotatingFillMatrix.GetRowChange(direction);
            int colChange = RotatingFillMatrix.GetColChange(direction);
            int cellValue = 1;
            ...
            while (cellValue <= this.Size * this.Size)
            {
                ...
                if (!this.matrix.CanContinueInDirection(position.Row + rowChange, position.Col + colChange))
                {
                    ...
                    rowChange = RotatingFillMatrix.GetRowChange(direction);
                    colChange = RotatingFillMatrix.GetColChange(direction);
